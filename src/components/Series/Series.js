...
const Series = ({ htmlEnd, series, seriesPosts }: Props) => {
  ...
  nodes.sort((a, b) => seriesSlugs.indexOf(a.frontmatter.slug) - seriesSlugs.indexOf(b.frontmatter.slug));
  ...
  return (
    <Post
      ...
      contentFooter={
        <div>
          {nodes.map((node, i) => (
            <SeriesPost key={node.frontmatter.title} {...node.frontmatter} {...node.fields} n={i + 1} />
          ))}
          <div
